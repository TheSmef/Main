...
import com.example.chat.databinding.ActivitySingUpBinding;
...
public class SingUpActivity extends AppCompatActivity {
    ...
    private String encodedImage(Bitmap bitmap){
        ...
        Bitmap previewBitmap = Bitmap.createScaledBitmap(bitmap, previewWidth, previewWidth, false);
    ...
    private Boolean isValidSignUpDetails(){
        if (encodedImage == null){
            ...
            showToast(getString(R.string.Confirm_password_and_password));
            return false;
        }
        else if(!Pattern.compile("^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z!@#$%^&*]{8,30}$").matcher(binding.inputPassword.getText().toString()).matches()){
